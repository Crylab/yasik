...
class YasikEvaluator(YasikListener):
    ...
    def exitFunctionReadingCall(self, ctx: YasikParser.FunctionReadingCallContext):
    ...
    def exitFunctionWritingCall(self, ctx: YasikParser.FunctionWritingCallContext):    
        parameter_name = ctx.getChild(0).getText()
        if ctx.getChildCount() == 6:
            column = ctx.getChild(2).getText()
            row = ctx.getChild(4).getText()
